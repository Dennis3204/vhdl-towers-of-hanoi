...
* `v_sync` — used as the “game tick” (updates once per frame) - (starter)
* `pixel_row[10:0]`, `pixel_col[10:0]` — current pixel coordinates from `vga_sync` - (starter)
* `arrow_x[10:0]` — selector x-position (from `towers.vhd`)
* `btn0` — pick/drop action

### Outputs

* `red`, `green`, `blue` — **1-bit** “is this pixel on?” signals - (starter)
...
* `clk_in1` — 100 MHz board clock - (starter)

### Outputs

* `clk_out1` — pixel clock - (starter)
